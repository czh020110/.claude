...
```
/sync-claude-introduction pull     从远程拉取项目记忆到本地
/sync-claude-introduction push     把本地项目记忆推送到远程
/sync-claude-introduction info     查看同步状态（本地/远程项目记忆是否有更新）
```
...
```bash
# 配置项目记忆仓库 URL（写入 .claude/.cache/docs-sync.conf，不进 git）
bash .claude/skills/sync-claude-introduction/scripts/sync-introduction.sh config <URL>
...
"未配置项目记忆仓库 URL"提示）。详细前置配置见仓库内 `.claude/skills/sync-claude-introduction/` 的相关说明。
