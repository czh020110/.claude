---
name: query-project
description: 当用户要求查询项目背景、项目边界、项目目标、环境、TODO，或你需要按问题从 `.project-memory/` 项目记忆文件中检索上下文时使用；委托 `query-project` agent 执行 RAG 检索并返回相关文档块摘要 (rag已配置)
---

基于 `.project-memory/` 下的项目记忆文件做只读 RAG 检索。
