...
- 完成阶段成果后，如需沉淀本次变更说明，使用 `update-docs` skill 同步长期文档与 git 提交说明；只有用户要求调整长期方向时才直接改 `STEP.md`；过程中用户提出的后续待办写入 `.project-memory/TODO/TODO.md` 共享待办列表；完成的待办按规则迁入 `DONE.md`（注明验证方式），不要交给 `update-docs` agent。
