...
- 适用于 `.project-memory/TODO/TODO.md` 共享待办列表的维护规则（由你和用户共同使用）。
...
### 3.4 共享待办 TODO.md 维护规则（MUST）

TODO.md 是你与用户共享的待办列表，结构只有简单 markdown 待办行，**不使用任何多级 `#` 标题、不分区**：

- 状态标记：`- [ ]` 计划中、`- [-]` 进行中、`- [x]` 已完成。
- 用户说"后续要做 X / 记个待办"（任何粒度）时，你主动在列表末尾追加 `- [ ] X`。
- 开始执行某条待办时把 `[ ]` 改为 `[-]`；完成后改为 `[x]`。
- 已完成条目最多保留最近 5 条；超过 5 条时你必须删除更早的已完成条目（也可以在确认用户不再需要后直接删除单条已完成项）。
- 该文件由你和用户直接编辑维护，不使用任何脚本或 skill 控制。
