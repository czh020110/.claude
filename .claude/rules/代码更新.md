...
4. 根据任务读取 `.claude_introduction/` 中的相关项目记忆。
...
- `.project-script` 目录仅存放测试相关文件（代码或产物），非测试代码不得放入。
...
根据变更类型选择验证方式，不能空口说完成，需要使用现成的或新建的测试脚本进行验证代码变更后的正确性：

- 对于后续**可复用**的验证脚本，放入`.project-script`目录，供后续迭代使用。
  - 不同类型验证脚本需自行放入`.project-script` 中的不同子目录，而非其根目录。如`.project-script/train`、`.project-script/test`、`.project-script/verify`、`.project-script/cli`等。
- 对于不可复用的一次性验证脚本，任务完成后需要清理掉。
...
- `.claude_introduction/TODO/TODO.md` 由主模型在执行任何与项目相关的实现、修改、验证任务前通过 `update-todo` skill 主动检查并按需维护，不直接手改任务条目。
