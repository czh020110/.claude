...
- 长期计划： @../.claude_introduction/TODO/STEP.md
  ...
  - 开发过程中用户提出新长期方向/阶段步骤/大方向调整，或主 agent 判断当前 STEP 已与任务脱节时，主 agent 应主动判断是否需要更新并在必要时与用户确认后直接修改 STEP.md；不交给 `update-docs` agent。
...
每轮回答或执行完成后，静默审视本轮是否产生需要持久化的内容，按顺序处理。**不打断用户，完成后不汇报**（除非与既有规则冲突需提醒）：

1. **新用户偏好**：出现工作方式、协作习惯、设计取向的新表达 → 按 `.claude/rules/文档更新.md` §2.1 直接归入对应主题正文，并在该主题 `MEMORY.md` 登记索引。
