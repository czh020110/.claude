...
2. **执行**：
   - 判断是否需要使用 `Task` 工具拆解任务。是，则执行：「Task 拆解流程」。
...
- 用户表示需要处理待办中的任务，或没有明确任务上下文但说“继续处理任务/待办”时，使用 `AskUserQuestion` 询问需要执行 TODO.md 中的哪一项，并给出推荐选项。
- 仅当当前任务属于 TODO 中的某项时更新其状态（`[ ]` 未完成 / `[-]` 进行中 / `[x]` 已完成）；临时任务不写入 TODO、不更新 TODO、不归档。

## Task 拆解流程（可分解任务 MUST）

仅用于真正多步骤（通常 3 步以上）或复杂任务；单步任务不建清单。符合条件时，使用 `Task` 工具管理执行步骤：

1. 仅在需要时检查 `.project-memory/TODO/TODO.md`（用户待办），然后使用 `Task` 列出本任务的步骤清单；临时任务不写入 TODO。
2. 按清单顺序推进任务；每完成一步、调整步骤或发现新步骤时，实时更新 `Task` 中的进度状态。
3. `Task` 只记录本任务的拆解方案和执行步骤，不写入 TODO.md；TODO.md 只记录用户任务本身，不记录任务拆解、执行步骤和方案。

完成阶段性工作后，检查 `Task` 中的任务状态；已完成项立即标记，不遗留未完成项。
...
当任务过程中明确需要用户意见时（例如记忆冲突、架构方向错误、设计取舍、关键设计选择），若 `AskUserQuestion` 可用，则提供几个选项让用户选择；若该工具不可用或无法交互，则自行基于已有信息判断处理后继续，并在任务结束时明确汇报本次使用了哪些假设和决策、以及需要用户后续确认的事项。
