...
- Documents： @../.project-memory/Documents/MEMORY.md
  ...
  - 默认只读；除非用户明确要求补充长期项目事实，不要主动写入正文，只可由update docs agent更新索引。
...
1. **新用户偏好**：出现工作方式、协作习惯、设计取向的新表达 → 直接归入对应主题正文，并在该主题 `MEMORY.md` 登记索引。（使用update-memory skill更新记忆）
2. **新设计决定**：用户提出方案变更、架构调整、边界调整或打算后续使用的新方向 → 先分析、确认后再直接修改对应正文并同步索引。（使用update-memory skill更新记忆）
...
6. **任务废弃**：用户明确放弃或方案变更导致待办失效时，同样把该条从 `TODO.md` 迁入 `DONE.md`，改 `- [x]` 并在行尾小括号标 `（放弃任务）`；任务只需变更微调时，直接在 TODO.md 原条目上实时修改。

无可沉淀内容时直接结束，不做多余动作。更新`.project-memory/TODO/`时无需使用skill。
