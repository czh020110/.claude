...
- 禁止在 `introduction/` 创建 `00-如何填写.md`、`template.md`、`00-生成规则.md`。
- 修改记录按一次 git commit 维度记录，不按文件分别记录。
- 修改记录必须详细说明修改原因、修改前状态、修改后状态、关键函数/接口/文档项、影响范围和验证结果。
...
- `introduction/项目文档/` 是用户维护的长期项目文档目录，只能读取作为上下文；除非用户明确要求，否则不要主动写入。
...
8. 运行验证。
9. 用户要求提交时，优先提交全部已更改，而不是部分提交；但如果仓库里存在明显不属于本次文档工作的改动，要先在结果中报告冲突，不要自行提交。
