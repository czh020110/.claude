...
- The agent's first phase proactively checks topic boundaries, sub-topic naming granularity and index/body structure across `.project-memory/` using only the memory-format spec, indexes and bodies. Every child name must be narrower than its parent; it may not repeat the parent topic at the same granularity. The agent may move, split, merge, rename or create topic bodies when the spec supports the change; fact and design statements remain verbatim, while file paths, index routing descriptions and purely structural headings may be narrowed to reflect their actual scope. It does not inspect code or diffs until this phase is complete. In the second phase it syncs current facts from commits and local changes. It does not touch `TODO/`, `.project-script/`, code or configuration, and creates no commits. `Plan/` designs may never be changed, reworded or deleted; `Pitfalls/` may be restructured in phase one but is not fact-synced in phase two.
