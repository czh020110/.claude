...
- The agent may restructure memory topics when the shared spec supports it. Keep fact and design statements verbatim; file paths, index descriptions and structural headings may be narrowed to match their contents. Do not alter or delete `Plan/` designs, touch `TODO/`, `.project-script/`, code or configuration, create commits, or sync `Pitfalls/` facts.
