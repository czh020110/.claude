...
This script reads the **index content** (each topic's `MEMORY.md`, `Plan/` included) plus `TODO/TODO.md`; it never reads topic body files.
...
The script outputs the full content of each topic's `MEMORY.md` — including `Plan/MEMORY.md`, the index of the design the project is meant to have — and of `TODO/TODO.md` (path + content). Based on the index's coverage, key objects and read conditions, combined with the current task, decide which body files to read next; read no bodies when no topic matches.
