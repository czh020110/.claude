...
detect_project_id() {
  ...
  local settings_file="$project_dir/.codex/settings.local.json"
  if [ -f "$settings_file" ]; then
    if command -v jq &>/dev/null; then
      id=$(jq -r '.CODEX_DOCS_PROJECT_ID // empty' "$settings_file" 2>/dev/null)
    else
      id=$(grep -o '"CODEX_DOCS_PROJECT_ID"[[:space:]]*:[[:space:]]*"[^"]*"' "$settings_file" 2>/dev/null | sed 's/.*: *"\(.*\)"/\1/')
    fi
    [ -n "$id" ] && { echo "$id"; return; }
  fi

  # 2. .codex/project-id 文件
  if [ -f "$project_dir/.codex/project-id" ]; then
    id=$(head -1 "$project_dir/.codex/project-id" | tr -d '[:space:]')
    [ -n "$id" ] && { echo "$id"; return; }
  fi
...
main() {
  ...
  if [ -z "$project_id" ]; then
    err "无法确定项目标识"
    err "请在 .codex/settings.local.json 中设置 CODEX_DOCS_PROJECT_ID"
    err "或创建 .codex/project-id 文件"
    exit 1
  fi
  echo "项目标识: $project_id"
  ...
  if [ -z "$repo_url" ]; then
    ...
    echo "  bash .codex/skills/sync-codex-memory/scripts/sync-memory.sh config https://github.com/youruser/codex-project-docs.git"
    echo ""
