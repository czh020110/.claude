...
4. Before syncing, check that the custom-prompt marker `<!-- sync-project-config:custom-prompts -->` is present in the target project's `AGENTS.md`; if it is, sync normally. When it is absent, the whole file is overwritten instead of a custom section being preserved.
