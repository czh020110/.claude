...
2. **执行**：
   - 判断是否需要使用 `update_plan/TodoWrite` 拆解任务。是，则执行：「update_plan/TodoWrite 拆解流程」。
   - 判断是否属于 TODO.md 列表。是，则执行：「TODO 待办处理」。
...
## update_plan/TodoWrite 拆解流程（可分解任务 MUST）

仅用于真正多步骤（通常 3 步以上）或复杂任务；单步任务不建清单。符合条件时，使用 `update_plan/TodoWrite` 工具管理执行步骤：

1. 仅在需要时检查 `.project-memory/TODO/TODO.md`（用户待办），然后使用 `update_plan/TodoWrite` 列出本任务的步骤清单；临时任务不写入 TODO。
2. 按清单顺序推进任务；每完成一步、调整步骤或发现新步骤时，实时更新 `update_plan/TodoWrite` 中的进度状态。
3. `update_plan/TodoWrite` 只记录本任务的拆解方案和执行步骤，不写入 TODO.md；TODO.md 只记录用户任务本身，不记录任务拆解、执行步骤和方案。

完成阶段性工作后，检查 `update_plan/TodoWrite` 中的任务状态；已完成项立即标记，不遗留未完成项。
...
当任务过程中明确需要用户意见时（例如记忆冲突、架构方向错误、设计取舍、关键设计选择），询问用户并给出选项；若当前环境无法交互，则自行基于已有信息判断处理后继续，并在任务结束时明确汇报本次使用了哪些假设和决策、以及需要用户后续确认的事项。

## 本地验证脚本

`.project-script/` 是随基础配置同步到项目中的本地验证脚本目录，与 `.project-memory/` 的长期事实记忆职责不同。具体目录规范、索引同步与安全要求由 `post-verify` skill 统一管理，执行验证时按该 skill 处理。
...
- 迁移、移除或重构后，不应保留“原版本包含…本版本已移除”之类的记录，除非该事实对后续维护仍有约束作用。
...
任务完成后必须执行 `post-verify` skill 完成最终验证；具体验证流程、脚本复用/创建与索引同步规则均在该 skill 中。
