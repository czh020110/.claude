...
- `.project-memory/` 只记录当前已经采用、实施并可验证的项目事实；不得写入未实施计划、候选方案、待定决策、用户提出但尚未落地的修改要求或实现路线。
- 讨论产生尚未实施的方案/设计决策时，主模型必须先更新 `TODO/Pending.md`；这一步不调用 `update-memory`，也不把内容复制到任何事实正文。
- Pending 项目完成实现并通过验证后，先移除对应 Pending 条目，再使用 `update-memory` 将最终真实方案写入事实正文；如果方案被否决或取消，只移除 Pending，不写入事实记忆。
- `update-memory` 只处理当前事实、已生效约束、环境/命令事实和可复用偏好；没有这类事实就不调用。
- 用户主动要求阶段性全量同步时才使用 `collect-update-memory`；其 commit 顺序、基准 commit 和同步边界以该 skill/agent 的专用说明为准，Pending 不由它转写为事实。
