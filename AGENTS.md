...
2. **执行**：
   - 判断是否需要使用功能update_plan拆解任务。是，则执行：「update_plan 拆解流程」。
...
- 用户表示需要处理待办中的任务，或没有明确任务上下文但说“继续处理任务/待办”时，使用 `request_user_input` 询问需要执行 TODO.md 中的哪一项，并给出推荐选项。
...
当任务过程中明确需要用户意见时（例如记忆冲突、架构方向错误、设计取舍、关键设计选择），若 `request_user_input` 可用，则提供几个选项让用户选择；若该工具不可用或无法交互，则自行基于已有信息判断处理后继续，并在任务结束时明确汇报本次使用了哪些假设和决策、以及需要用户后续确认的事项。
