...
# ============================ 远程项目记忆仓库 URL 检测 ============================ #
#
# 唯一配置位置： .codebuddy/.cache/docs-sync.conf（一行纯文本 URL）
# 通过 config 子命令写入，不进 git，跨设备需各自配置一次。

detect_remote_repo() {
  local project_dir="$1"
  local cache_conf="$project_dir/.codebuddy/.cache/docs-sync.conf"
...
# 将 URL 写入  .codebuddy/.cache/docs-sync.conf
write_remote_repo() {
  local project_dir="$1" url="$2"
  local cache_dir="$project_dir/.codebuddy/.cache"
  if ! mkdir -p "$cache_dir" 2>/dev/null; then
    err "创建失败: $cache_dir"
    err "请确认 .codebuddy/.cache 目录存在且当前用户可写"
    return 1
  fi
...
main() {
  ...
  if [ -z "$repo_url" ]; then
    ...
    echo "  bash .codebuddy/skills/sync-project-memory/scripts/sync-memory.sh config https://github.com/youruser/codex-project-docs.git"
    echo ""
